...
class TestCardActionCallbackResponse:
    ...
    def test_returns_card_for_approve_action_value_json_string(self, _patch_callback_card_types):
        adapter = _make_adapter()
        adapter._loop = MagicMock()
        adapter._loop.is_closed = MagicMock(return_value=False)
        adapter._allowed_group_users = {"ou_bob"}
        ...
        data = _make_card_action_data(
            json.dumps({"hermes_action": "approve_once", "approval_id": 7}),
            open_id="ou_bob",
        )
        adapter._sender_name_cache["ou_bob"] = ("Bob", 9999999999)
    ...
    def test_returns_card_for_approve_action_value_namespace(self, _patch_callback_card_types):
        adapter = _make_adapter()
        adapter._loop = MagicMock()
        adapter._loop.is_closed = MagicMock(return_value=False)
        adapter._allowed_group_users = {"ou_bob"}
        ...
        data = _make_card_action_data(
            SimpleNamespace(approval_id="9", hermes_action="approve_once"),
            open_id="ou_bob",
        )
        adapter._sender_name_cache["ou_bob"] = ("Bob", 9999999999)
    ...
    def test_returns_card_for_update_prompt_value_json_string(self, _patch_callback_card_types):
        adapter = _make_adapter()
        adapter._loop = MagicMock()
        adapter._loop.is_closed = MagicMock(return_value=False)
        ...
        data = _make_card_action_data(
            json.dumps({"hermes_update_prompt_action": "y", "update_prompt_id": 8}),
            open_id="ou_bob",
        )
        adapter._sender_name_cache["ou_bob"] = ("Bob", 9999999999)
