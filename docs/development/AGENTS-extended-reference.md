...
This file preserves the long-form details that were intentionally moved out of
`AGENTS.md` so the auto-loaded context file stays below Hermes' per-file prompt
limit. `AGENTS.md` remains the source of always-loaded rules; use this file only
when you need expanded examples or implementation notes for the summarized
sections.
