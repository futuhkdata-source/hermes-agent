...
def _active_skills_dir() -> Path:
    """Resolve the local skills root for the active profile runtime scope.

    ``tools.skills_tool`` is imported once by the long-lived gateway, before a
    routed profile is selected.  Keep honoring tests/callers that monkeypatch
    ``SKILLS_DIR``, but otherwise resolve from the context-local HERMES_HOME on
    every call so profile-local skills do not fall back to the default profile.
    """
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    """
    # Resolve the context-local profile root first, then external dirs.
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    # Scan the active profile's local dir first, then external dirs.
